...
Az alapértelmezés szerinti installáláskor a bitcoind a _/usr/local/bin_ könyvtárba kerül. Amikor a bitcoindt először futtatjuk, akkor üzen, hogy egy konfigurációs állományt kell létrehoznunk, melyben a JSON-RPC interface számára egy erős jelszó van megadva. Futtassunk a bitcoindt a +bitcoind+ terminálon történő begépelésével:
